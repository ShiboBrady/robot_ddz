...
{
    handMap.clear();
    getBomb(cards, handMap);//获取炸弹
    getTrioChain(cards, handMap);//获取飞机
    getSoloChain(cards, handMap);//获取顺子
    getPairChain(cards, handMap);//获取连对
    getNormalHand(cards, handMap);
    if (searchKicker)
    {
        findKickerForTrioOrTrioChain(handMap);
                                   ...
                                   Hand *hand)
{
    ...
    else if (hand->type = TRIO_CHAIN_SOLO)
    {
        vector<int> k;
        vector<Hand> &soloHands = handsMap[SOLO];
        for (unsigned i=0; i<soloHands.size(); ++i)
        ...
        if (k.size() < (unsigned)hand->len)
        {
            vector<Hand> &soloChainHands = handsMap[SOLO_CHAIN];
            for (unsigned i=0; i<soloChainHands.size(); ++i)
            {
                while (k.size() < (unsigned)hand->len && soloChainHands[i].len > 5)
                {
                    if (soloChainHands[i].keyPoint < hand->keyPoint
                        || soloHands[i].keyPoint > hand->keyPoint + hand->len - 1)
                    {
    ...
    else if (hand->type = TRIO_CHAIN_PAIR)
    {
        vector<int> k;
        vector<Hand> &pairHands = handsMap[PAIR];
        ...
        if (k.size() < (unsigned)hand->len)
        {
            ...
            for (unsigned i=0; i<pairChainHands.size(); ++i)
            {
                while (k.size() < (unsigned)hand->len && pairChainHands[0].len > 3)
                {
...
{
    hand->type = NOTHING;
    if (points[BLACK_JOKER] == 1 && points[RED_JOKER] == 1)//出得如果是双王
    {
    ...
    else
    {
        ...
        for (int i=CARD_3; i<=RED_JOKER; ++i)//将出牌信息放入二维数组sortedCard中，一维表示张数，二维表示类型
        {
            if (points[i] > 0)
            {
