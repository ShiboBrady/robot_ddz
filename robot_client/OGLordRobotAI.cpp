...
bool OGLordRobotAI::RbtInInitCard(int argSeat, std::vector<int> argHandCard)
{
	...
	for (int i=0; i<PLAYER_NUM; ++i)//该循环设置自己的牌，同时初始化其他人的牌为-1
	{
	...
	callHistory.clear();
	history.clear();
	sortHandMap();//将手中的牌按照最优的方式排列，这将成为以后出牌的依据
...
bool OGLordRobotAI::RbtOutGetCallScore(int &callScore)
{
	...
	if (controlNum + summary.extraBombCount <= 1)
	{
		...
		{
			if (callHistory.size() == 2 && curScore == 0)
			{
				isCallLord = true;
			}
		}
...
{
	lowestControl = getLowestControl(remainPoints);//返回其他人手中所有牌中比某张牌大的牌还有6张以上的那种牌
	controlNum = calControl(playerInfo[aiSeat].points, otherPoints, lowestControl);
	Hand hand;
	if (level == 0)
	{
...
void OGLordRobotAI::sortHandMap()
{
	maxOppNumber = 0;
	minOppNumber = 20;
	if (lordSeat == -1 || lordSeat == aiSeat)//刚发牌，或者是自己是地主
	{
		for (int i=0; i<PLAYER_NUM; ++i)
		{
			if (i != aiSeat)
...
void OGLordRobotAI::lordTakeOutFreeFarmerOnly1Card( Hand &hand )
{
		...
		&& handsMap[BOMB].size() > 0)
	{
		...
		hand.keyPoint = handsMap[BOMB][0].keyPoint;
	...
	else
	{
		if (curHandSeat == lordSeat)
		{
			if (curHand.type == SOLO)
			{
				...
				for (int i=max(curHand.keyPoint + 1, oppHighest); i<=RED_JOKER; ++i)
				{
					if (aiPoints[i] > 0)
					{
...
void OGLordRobotAI::farmerTakeOutLordOnly2Cards( Hand &hand )
{
	...
	else
	{
		if (curHand.type == PAIR && curHandSeat == lordSeat)
		{
			...
			{
				if (pairHands[i].keyPoint >= max(curHand.keyPoint + 1, (int)highestPair))
				{
					hand = pairHands[i];
					return;
				}
...
void OGLordRobotAI::upBadFarmerTakeOut( Hand &hand )
{
	...
	else
	{
		...
		if (curHand.type == SOLO
			&& curHand.keyPoint < min(CARD_J, lowestControl))
		{
			int higherP = max((int)CARD_J, curHand.keyPoint + 1);
			vector<Hand> &soloHands = handsMap[SOLO];
			for (unsigned i=0; i<soloHands.size(); ++i)
			{
...
bool OGLordRobotAI::isDangerLvl()
{
	...
	if (aiSeat == lordSeat)
	{
		highBomb = max(getHighestBomb(playerInfo[upSeat].points),
													 ...
													 int trioCount )
{
	...
	if (lowSoloCount > 0)
	{
			...
			&& goodLowHands[TRIO] < uniHighHands[TRIO])
		{
			...
			{
				lowSoloCount = 0;
				lowPairCount = max(lowPairCount - 1, 0);
			...
			{
				lowSoloCount = max(lowSoloCount - 3, 0);
		...
		{
			lowSoloCount = max(lowSoloCount - 2, 0);
